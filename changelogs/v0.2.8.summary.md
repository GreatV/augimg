...
Previous versions of `augimg` were primarily geared towards `uint8`.
...
is now listed in the documentation on page [dtype support](https://augimg.readthedocs.io/en/latest/source/dtype_support.html).
...
with aug.pool(processes=-1, seed=123) as pool:
    ...
    for batch_aug in batches_aug:
        # do something
```
Here, `batches_generator` is a generator that yields instances of `augimg.Batch`, e.g. something like `augimg.Batch(images=<numpy array>, keypoints=[augimg.KeypointsOnImage(...), augimg.KeypointsOnImage(...), ...])`. The arguement `processes=-1` spawns `N-1` workers, where `N` is the number of CPU cores (includes hyperthreads).
...
* Improved performance of `pad()`
    * this change also improves the performance of:
        * `augimg.augimg.pad_to_aspect_ratio()`,
        * `augimg.augimg.HeatmapsOnImage.pad()`,
        * `augimg.augimg.HeatmapsOnImage.pad_to_aspect_ratio()`,
        * `augimg.augimg.SegmentationMapOnImage.pad()`,
        * `augimg.augimg.SegmentationMapOnImage.pad_to_aspect_ratio()`,
        * `augimg.augmenters.size.PadToFixedSize`,
        * `augimg.augmenters.size.Pad`,
        * `augimg.augmenters.size.CropAndPad`
...
* Fixed an issue with `SEED_MAX_VALUE` exceeding the `int32` maximum on some systems, causing crashes related to
  RandomState.
* Moved BatchLoader to `multicore.py` and replaced the class with an alias pointing to `augimg.multicore.BatchLoader`.
* Moved BackgroundAugmenter to `multicore.py` and replaced the class with an alias pointing to `augimg.multicore.BatchLoader`.
...
* Renamed in `augimg.Batch` the attributes storing input data `<attribute>_unaug`, e.g. `augimg.Batch.images` to `augimg.Batch.images_unaug` or `augimg.Batch.keypoints` to `augimg.Batch.keypoints_unaug`. The old attributes are still accessible, but will raise a DeprecatedWarning.


## augimg.multicore

* Created this file.
* Moved `BatchLoader` here from `augimg.py`.
* Moved `BackgroudAugmenter` here from `augimg.py`.
...
* Added new file `augimg/dtypes.py` and respective test file `test_dtypes.py`.
...
* Added `pool()` to `Augmenter`. This is a helper to start a `augimg.multicore.Pool` via `with augmenter.pool() as pool: ...`.
* Refactored `Augmenter.augment_batches(..., background=True)` to use `augimg.multicore.Pool`.
...
* Renamed `augimg.augmenters.overlay` to `augimg.augmenters.blend`. Functions and classes in `augimg.augmenters.overlay` are still accessible, but will now raise a DeprecatedWarning.
* Added `blend_alpha()`.
...
## augimg.augmenters.convolutional
