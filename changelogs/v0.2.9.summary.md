...
added. The previous file for that was `augimg/augimg.py`, which however was
already fairly large. Therefore, all classes and methods related to augmentable
data were split off and moved to `augimg/augmentables/<type>.py`. The new
modules and their main contents are:
* `augimg.augmentables.batches`: Contains `Batch`, `UnnormalizedBatch`.
* `augimg.augmentables.utils`: Contains utility functions.
* `augimg.augmentables.bbs`: Contains `BoundingBox`, `BoundingBoxesOnImage`.
* `augimg.augmentables.kps`: Contains `Keypoint`, `KeypointsOnImage`.
* `augimg.augmentables.polys`: Contains `Polygon`, `PolygonsOnImage`.
* `augimg.augmentables.lines`: Contains `LineString`, `LineStringsOnImage`.
* `augimg.augmentables.heatmaps`: Contains `HeatmapsOnImage`.
* `augimg.augmentables.segmaps`: Contains `SegmentationMapOnImage`.

Currently, all augmentable classes can still be created via `augimg.<type>`,
e.g. `augimg.BoundingBox` still works.

Changes related to the new modules:
* Moved `Keypoint`, `KeypointsOnImage` and `augimg.augimg.compute_geometric_median` to `augmentables/kps.py`.
...
* Added module `augimg.augmentables.utils`.
    * Added function `normalize_shape()`.
    * Added function `project_coords()`.
* Moved line interpolation functions `_interpolate_points()`, `_interpolate_point_pair()` and `_interpolate_points_by_max_distance()` to `augimg.augmentables.utils` and made them public functions.
* Refactored `__init__()` of `PolygonsOnImage`, `BoundingBoxesOnImage`, `KeypointsOnImage` to make use of `augimg.augmentables.utils.normalize_shape()`.
* Refactored `KeypointsOnImage.on()` to use `augimg.augmentables.utils.normalize_shape()`.
* Refactored `Keypoint.project()` to use `augimg.augmentables.utils.project_coords()`.

## Polygon Augmentation

Polygons were already part of `augimg` for quite a while, but couldn't be
...
See [augimg-doc/notebooks](https://github.com/greatv/augimg-doc/tree/master/notebooks)
for a jupyter notebook with many more examples.

Changes related to polygon augmentation:
* Added `_ConcavePolygonRecoverer` to `augimg.augmentables.polys`.
* Added `PolygonsOnImage` to `augimg.augmentables.polys`.
* Added polygon augmentation methods:
    * Added `augment_polygons()` to `Augmenter`.
    * Added `_augment_polygons()` to `Augmenter`.
    * Added `_augment_polygons_as_keypoints()` to `Augmenter`.
    * Added argument `polygons` to `augimg.augmentables.batches.Batch`.
    * Added attributes `polygons_aug` and `polygons_unaug` to `augimg.augmentables.batches.Batch`.
...
* Added `augimg.quokka_polygons()` function to generate example polygon data.
...
* `list(list(augimg.augmentables.polys.Polygon))` for polygons.
  Note that this "skips" `augimg.augmentables.polys.PolygonsOnImage`.
...
which will return an instance of `augimg.augmentables.batches.UnnormalizedBatch`.
...
* Added `augimg.augmentables.batches.UnnormalizedBatch`.
* Added module `augimg.augmentables.normalization` for data normalization routines.
...
* Added `augimg.augimg.DeprecationWarning`. The builtin python `DeprecationWarning` is silent since 2.7, which is why now a separate deprecation warning is used.
* Added `augimg.augimg.warn_deprecated()`.
    * Refactored deprecation warnings to use this function.
* Added `augimg.augimg.deprecated` decorator.
...
* Added function `augimg.augimg.flatten()` that flattens nested lists/tuples.
...
* Refactored `augimg.augmenters.blend.blend_alpha()` to no longer enforce a channel axis for foreground and background image.
* Refactored `augimg/parameters.py` to reorder classes within the file.
