...
class ArgCopyingMagicMock(mock.MagicMock):
    ...
    This is useful for augimg as many augmentation methods change data
    in-place.
...
def is_parameter_instance(param, param_type):
    ...
    is an instance of :class:`augimg.parameters.AutoPrefetcher`, then it
    ...
    Parameters
    ----------
    param : augimg.parameters.StochasticParameter
        The parameter to check.

    param_type : type
        The desired type. Similar as in ``isinstance``.
        E.g. ``augimg.parameters.Deterministic``.
