...
def _quokka_normalize_extract(extract):
    ...
    Added in 0.5.0. (Moved from ``augimg.augimg``.)

    Parameters
    ----------
    extract : 'square' or tuple of number or augimg.augmentables.bbs.BoundingBox or augimg.augmentables.bbs.BoundingBoxesOnImage
        Unnormalized representation of the image subarea to be extracted.
            ...
            * If a :class:`~augimg.augmentables.bbs.BoundingBox`, then that
              bounding box's area will be extracted from the image.
            * If a :class:`~augimg.augmentables.bbs.BoundingBoxesOnImage`,
...
def quokka(size=None, extract=None):
    ...
    size : None or float or tuple of int, optional
        ...
        :func:`~augimg.augimg.imresize_single_image`. Usually expected to be a
        ``tuple`` ``(H, W)``, where ``H`` is the desired height and ``W`` is
        the width. If ``None``, then the image will not be resized.

    extract : None or 'square' or tuple of number or augimg.augmentables.bbs.BoundingBox or augimg.augmentables.bbs.BoundingBoxesOnImage
...
def quokka_keypoints(size=None, extract=None):
    ...
    extract : None or 'square' or tuple of number or augimg.augmentables.bbs.BoundingBox or augimg.augmentables.bbs.BoundingBoxesOnImage
        Subarea to extract from the image. See :func:`~augimg.augimg.quokka`.
    ...
    """
    # TODO get rid of this deferred import
    from augimg.augmentables.kps import Keypoint, KeypointsOnImage
