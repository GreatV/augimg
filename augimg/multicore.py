...
def _get_context_method():
    ...
    if "NixOS" in platform.version():
        ...
        if get_context_unsupported:
            ia.warn("Detected usage of augimg.multicore in python <=3.4 "
...
class Pool(object):
    ...
    Parameters
    ----------
    augseq : augimg.augmenters.meta.Augmenter
    ...
    def map_batches(self, batches, chunksize=None):
        ...
        Parameters
        ----------
        batches : list of augimg.augmentables.batches.Batch
        ...
        Returns
        -------
        list of augimg.augmentables.batches.Batch
            Augmented batches.
    ...
    def imap_batches(self, batches, chunksize=1, output_buffer_size=None):
        ...
        Parameters
        ----------
        batches : generator of augimg.augmentables.batches.Batch
    ...
    def join(self):
        ...
        :func:`~augimg.multicore.Pool.close` or
        :func:`~augimg.multicore.Pool.terminate`.
...
def _Pool_worker(batch_idx, batch):
    ...
    assert isinstance(batch, (UnnormalizedBatch, Batch)), (
        ...
        "`augimg.augmentables.batches.UnnormalizedBatch` or "
        "`augimg.augmentables.batches.Batch`. Got type %s instead." % (
            type(batch)
        ))
...
class BatchLoader(object):
    ...
    Deprecated. Use ``augimg.multicore.Pool`` instead.

    Loaded batches can be accesses using :attr:`augimg.BatchLoader.queue`.
    ...
    @ia.deprecated(alt_func="augimg.multicore.Pool")
        ...
        try:
            ...
            for batch in gen:
                assert isinstance(batch, Batch), (
                    ...
                    "be of class augimg.Batch, got %s." % (
                        type(batch),))
