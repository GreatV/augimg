...
# because otherwise unittest seems to mix up augimg.augmenters.segmentation
...
from ..augimg import _NUMBA_INSTALLED, _numbajit
...
def _ensure_image_max_size(image, max_size, interpolation):
    ...
    **Supported dtypes**:
    ...
    See :func:`~augimg.augimg.imresize_single_image`.
...
class Superpixels(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    if (image size > max_size):
        ...
        minimum of (
            ``augimg.augmenters.segmentation.Superpixels(image size <= max_size)``,
            :func:`~augimg.augmenters.segmentation._ensure_image_max_size`
        )

    Parameters
    ----------
    p_replace : number or tuple of number or list of number or augimg.parameters.StochasticParameter, optional
    ...
    n_segments : int or tuple of int or list of int or augimg.parameters.StochasticParameter, optional
...
def segment_voronoi(image, cell_coordinates, replace_mask=None):
    ...
    **Supported dtypes**:
    ...
    if (image size > max_size):
        ...
        minimum of (
            ``augimg.augmenters.segmentation.Voronoi(image size <= max_size)``,
            :func:`~augimg.augmenters.segmentation._ensure_image_max_size`
        )
...
class Voronoi(meta.Augmenter):
    ...
    **Supported dtypes**:
    ...
    See :func:`augimg.augmenters.segmentation.segment_voronoi`.
...
class UniformVoronoi(Voronoi):
    ...
    :class:`~augimg.augmenters.segmentation.Voronoi` with
    :class:`~augimg.augmenters.segmentation.UniformPointsSampler`. Hence, it
    ...
    **Supported dtypes**:
    ...
    See :class:`~augimg.augmenters.segmentation.Voronoi`.

    Parameters
    ----------
    n_points : int or tuple of int or list of int or augimg.parameters.StochasticParameter, optional
...
class RegularGridVoronoi(Voronoi):
    ...
    This augmenter is a shortcut for the combination of
    :class:`~augimg.augmenters.segmentation.Voronoi`,
    :class:`~augimg.augmenters.segmentation.RegularGridPointsSampler` and
    :class:`~augimg.augmenters.segmentation.DropoutPointsSampler`. Hence, it
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    n_rows : int or tuple of int or list of int or augimg.parameters.StochasticParameter, optional
    ...
    n_cols : int or tuple of int or list of int or augimg.parameters.StochasticParameter, optional
    ...
    p_drop_points : number or tuple of number or augimg.parameters.StochasticParameter, optional
        ...
        coordinate.
            ...
            * If a ``StochasticParameter``, then this parameter will be used to
              ...
              ``augimg.parameters.Binomial(1-p)`` to convert parameter `p` to
...
class RelativeRegularGridVoronoi(Voronoi):
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    n_rows_frac : number or tuple of number or list of number or augimg.parameters.StochasticParameter, optional
    ...
    n_cols_frac : number or tuple of number or list of number or augimg.parameters.StochasticParameter, optional
...
class IPointsSampler(object):
    ...
    def sample_points(self, images, random_state):
        ...
        random_state : None or int or augimg.random.RNG or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.SeedSequence or numpy.random.RandomState
            A random state to use for any probabilistic function required
            during the point sampling.
            See :func:`~augimg.random.RNG` for details.
...
class DropoutPointsSampler(IPointsSampler):
    ...
    p_drop : number or tuple of number or augimg.parameters.StochasticParameter
        The probability that a coordinate will be removed from the list
        of all sampled coordinates. A value of ``1.0`` would mean that (on
        average) ``100`` percent of all coordinates will be dropped,
        while ``0.0`` denotes ``0`` percent. Note that this sampler will
