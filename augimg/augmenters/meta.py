...
Note: :class:`~augimg.augmenters.color.WithColorspace` is in ``color.py``.
...
@ia.deprecated("augimg.dtypes.clip_")
...
class _maybe_deterministic_ctx(object):  # pylint: disable=invalid-name
    ...
    Parameters
    ----------
    random_state : augimg.random.RNG or augimg.augmenters.meta.Augmenter
...
class Augmenter(object):
    ...
    Parameters
    ----------
    seed : None or int or augimg.random.RNG or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.SeedSequence or numpy.random.RandomState, optional
        ...
        need to set this parameter.
            ...
            * If ``int``: The value will be used as a seed for a new
              :class:`~augimg.random.RNG` instance.
            * If :class:`~augimg.random.RNG`: The ``RNG`` instance will be
              used without changes.
            * If :class:`~augimg.random.Generator`: A new
              :class:`~augimg.random.RNG` instance will be
              created, containing that generator.
            * If :class:`~augimg.random.bit_generator.BitGenerator`: Will
              be wrapped in a :class:`~augimg.random.Generator`. Then
              similar behaviour to :class:`~augimg.random.Generator`
              parameters.
            * If :class:`~augimg.random.SeedSequence`: Will
              be wrapped in a new bit generator and
              :class:`~augimg.random.Generator`. Then
              similar behaviour to :class:`~augimg.random.Generator`
              parameters.
            * If :class:`~augimg.random.RandomState`: Similar behaviour to
              :class:`~augimg.random.Generator`. Outdated in numpy 1.17+.
    ...
    random_state : None or int or augimg.random.RNG or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.SeedSequence or numpy.random.RandomState, optional
    ...
    def __init__(self, seed=None, name=None,
        ...
        if deterministic != "deprecated":
            ia.warn_deprecated(
                ...
                "in `augimg.augmenters.meta.Augmenter`. Use "
    ...
    def augment_batches(self, batches, hooks=None, background=False):
        ...
        is activated, an instance of :class:`~augimg.multicore.Pool` will
        ...
        Parameters
        ----------
        batches : augimg.augmentables.batches.Batch or augimg.augmentables.batches.UnnormalizedBatch or iterable of augimg.augmentables.batches.Batch or iterable of augimg.augmentables.batches.UnnormalizedBatch
            A single batch or a list of batches to augment.

        hooks : None or augimg.HooksImages, optional
        ...
        Yields
        -------
        augimg.augmentables.batches.Batch or augimg.augmentables.batches.UnnormalizedBatch or iterable of augimg.augmentables.batches.Batch or iterable of augimg.augmentables.batches.UnnormalizedBatch
            Augmented batches.
            ...
            if isinstance(batch, Batch):
                ...
                if len(batch) == 0:
                    ...
                    raise Exception(
                        ...
                        "or augimg.HeatmapsOnImage or "
                        "augimg.SegmentationMapsOnImage or "
                        "augimg.KeypointsOnImage or "
                        "augimg.BoundingBoxesOnImage, "
                        "or augimg.PolygonsOnImage, "
                        "got %s." % (type(batch[0]),))
            else:
                raise Exception(
                    "Unknown datatype of batch. Expected augimg.Batch or "
                    "augimg.UnnormalizedBatch or "
                    "numpy array or list of (numpy array or "
                    "augimg.HeatmapsOnImage or "
                    "augimg.SegmentationMapsOnImage "
                    "or augimg.KeypointsOnImage or "
                    "augimg.BoundingBoxesOnImage or "
                    "augimg.PolygonsOnImage). Got %s." % (type(batch),))

            if batch_orig_dt not in ["augimg.Batch",
                                     "augimg.UnnormalizedBatch"]:
                ia.warn_deprecated(
                    "Received an input in augment_batches() that was not an "
                    "instance of augimg.augmentables.batches.Batch "
                    "or augimg.augmentables.batches.UnnormalizedBatch, but "
    ...
    def augment_batch_(self, batch, parents=None, hooks=None):
        ...
        Parameters
        ----------
        batch : augimg.augmentables.batches.Batch or augimg.augmentables.batches.UnnormalizedBatch or augimg.augmentables.batch._BatchInAugmentation
            A single batch to augment.

            If :class:`augimg.augmentables.batches.UnnormalizedBatch`
            or :class:`augimg.augmentables.batches.Batch`, then the ``*_aug``
            attributes may be modified in-place, while the ``*_unaug``
            attributes will not be modified.
            If :class:`augimg.augmentables.batches._BatchInAugmentation`,
            then all attributes may be modified in-place.

        parents : None or list of augimg.augmenters.Augmenter, optional
        ...
        Returns
        -------
        augimg.augmentables.batches.Batch or augimg.augmentables.batches.UnnormalizedBatch
            Augmented batch.
    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        ...
        Parameters
        ----------
        batch : augimg.augmentables.batches._BatchInAugmentation
            The normalized batch to augment. May be changed in-place.

        random_state : augimg.random.RNG
            The random state to use for all sampling tasks during the
            augmentation.

        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_batch_`.

        hooks : augimg.augimg.HooksImages or None
            See :func:`~augimg.augmenters.meta.Augmenter.augment_batch_`.

        Returns
        -------
        augimg.augmentables.batches._BatchInAugmentation
            The augmented batch.
    ...
    def augment_images(self, images, parents=None, hooks=None):
        ...
        hooks : None or augimg.augimg.HooksImages, optional
            :class:`~augimg.augimg.HooksImages` object to dynamically
    ...
    def _augment_images(self, images, random_state, parents, hooks):
        ...
        .. note::
            ...
            Overwriting :func:`~augimg.augmenters.meta.Augmenter._augment_batch`
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_images`.
    ...
    def augment_heatmaps(self, heatmaps, parents=None, hooks=None):
        ...
        Parameters
        ----------
        heatmaps : augimg.augmentables.heatmaps.HeatmapsOnImage or list of augimg.augmentables.heatmaps.HeatmapsOnImage
            Heatmap(s) to augment. Either a single heatmap or a list of
            heatmaps.

        parents : None or list of augimg.augmenters.meta.Augmenter, optional
            Parent augmenters that have previously been called before the
            call to this function. Usually you can leave this parameter as
            ``None``.
            It is set automatically for child augmenters.

        hooks : None or imaug.augimg.HooksHeatmaps, optional
            :class:`~augimg.augimg.HooksHeatmaps` object to dynamically
            interfere with the augmentation process.

        Returns
        -------
        augimg.augmentables.heatmaps.HeatmapsOnImage or list of augimg.augmentables.heatmaps.HeatmapsOnImage
    ...
    def _augment_heatmaps(self, heatmaps, random_state, parents, hooks):
        ...
        Parameters
        ----------
        heatmaps : list of augimg.augmentables.heatmaps.HeatmapsOnImage
            Heatmaps to augment. They may be changed in-place.

        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_heatmaps`.

        hooks : augimg.augimg.HooksHeatmaps or None
            See :func:`~augimg.augmenters.meta.Augmenter.augment_heatmaps`.

        Returns
        ----------
        images : list of augimg.augmentables.heatmaps.HeatmapsOnImage
    ...
    def augment_segmentation_maps(self, segmaps, parents=None, hooks=None):
        ...
        Parameters
        ----------
        segmaps : augimg.augmentables.segmaps.SegmentationMapsOnImage or list of augimg.augmentables.segmaps.SegmentationMapsOnImage
        ...
        hooks : None or augimg.HooksHeatmaps, optional
            :class:`~augimg.augimg.HooksHeatmaps` object to dynamically
            interfere with the augmentation process.

        Returns
        -------
        augimg.augmentables.segmaps.SegmentationMapsOnImage or list of augimg.augmentables.segmaps.SegmentationMapsOnImage
    ...
    def _augment_segmentation_maps(self, segmaps, random_state, parents, hooks):
        ...
        Parameters
        ----------
        segmaps : list of augimg.augmentables.segmaps.SegmentationMapsOnImage
            Segmentation maps to augment. They may be changed in-place.

        parents : list of augimg.augmenters.meta.Augmenter
            See
            :func:`~augimg.augmenters.meta.Augmenter.augment_segmentation_maps`.
        ...
        Returns
        ----------
        images : list of augimg.augmentables.segmaps.SegmentationMapsOnImage
    ...
    def augment_keypoints(self, keypoints_on_images, parents=None, hooks=None):
        ...
        Parameters
        ----------
        keypoints_on_images : augimg.augmentables.kps.KeypointsOnImage or list of augimg.augmentables.kps.KeypointsOnImage
            The keypoints/landmarks to augment.
            Either a single instance of
            :class:`~augimg.augmentables.kps.KeypointsOnImage` or a list of
        ...
        hooks : None or augimg.augimg.HooksKeypoints, optional
            :class:`~augimg.augimg.HooksKeypoints` object to dynamically
            interfere with the augmentation process.

        Returns
        -------
        augimg.augmentables.kps.KeypointsOnImage or list of augimg.augmentables.kps.KeypointsOnImage
            Augmented keypoints.
    ...
    def _augment_keypoints(self, keypoints_on_images, random_state, parents,
        ...
        Parameters
        ----------
        keypoints_on_images : list of augimg.augmentables.kps.KeypointsOnImage
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_keypoints`.

        hooks : augimg.augimg.HooksKeypoints or None
            See :func:`~augimg.augmenters.meta.Augmenter.augment_keypoints`.

        Returns
        ----------
        list of augimg.augmentables.kps.KeypointsOnImage
    ...
    def augment_bounding_boxes(self, bounding_boxes_on_images, parents=None,
        ...
        >>> import augimg.augmenters as iaa
        >>> from augimg.augmentables.bbs import BoundingBox
        >>> from augimg.augmentables.bbs import BoundingBoxesOnImage
        ...
        Parameters
        ----------
        bounding_boxes_on_images : augimg.augmentables.bbs.BoundingBoxesOnImage or list of augimg.augmentables.bbs.BoundingBoxesOnImage
            The bounding boxes to augment.
            Either a single instance of
            :class:`~augimg.augmentables.bbs.BoundingBoxesOnImage` or a list of
        ...
        Returns
        -------
        augimg.augmentables.bbs.BoundingBoxesOnImage or list of augimg.augmentables.bbs.BoundingBoxesOnImage
    ...
    def augment_polygons(self, polygons_on_images, parents=None, hooks=None):
        ...
        Parameters
        ----------
        polygons_on_images : augimg.augmentables.polys.PolygonsOnImage or list of augimg.augmentables.polys.PolygonsOnImage
            The polygons to augment.
            Either a single instance of
            :class:`~augimg.augmentables.polys.PolygonsOnImage` or a list of
        ...
        Returns
        -------
        augimg.augmentables.polys.PolygonsOnImage or list of augimg.augmentables.polys.PolygonsOnImage
            Augmented polygons.
    ...
    def augment_line_strings(self, line_strings_on_images, parents=None,
        ...
        >>> import augimg.augmenters as iaa
        >>> from augimg.augmentables.lines import LineString
        >>> from augimg.augmentables.lines import LineStringsOnImage
        >>> A = B = C = np.ones((10, 10), dtype=np.uint8)
        ...
        Parameters
        ----------
        line_strings_on_images : augimg.augmentables.lines.LineStringsOnImage or list of augimg.augmentables.lines.LineStringsOnImage
            The line strings to augment.
            Either a single instance of
            :class:`~augimg.augmentables.lines.LineStringsOnImage` or a list of
        ...
        Returns
        -------
        augimg.augmentables.lines.LineStringsOnImage or list of augimg.augmentables.lines.LineStringsOnImage
    ...
    def _augment_bounding_boxes(self, bounding_boxes_on_images, random_state,
        ...
        Parameters
        ----------
        bounding_boxes_on_images : list of augimg.augmentables.bbs.BoundingBoxesOnImage
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_bounding_boxes`.
        ...
        Returns
        -------
        list of augimg.augmentables.bbs.BoundingBoxesOnImage
    ...
    def _augment_polygons(self, polygons_on_images, random_state, parents,
        ...
        Parameters
        ----------
        polygons_on_images : list of augimg.augmentables.polys.PolygonsOnImage
            Polygons to augment. They may be changed in-place.
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_polygons`.
        ...
        Returns
        -------
        list of augimg.augmentables.polys.PolygonsOnImage
    ...
    def _augment_line_strings(self, line_strings_on_images, random_state,
        ...
        Parameters
        ----------
        line_strings_on_images : list of augimg.augmentables.lines.LineStringsOnImage
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_line_strings`.
        ...
        Returns
        -------
        list of augimg.augmentables.lines.LineStringsOnImage
    ...
    def _augment_bounding_boxes_as_keypoints(self, bounding_boxes_on_images,
        ...
        Parameters
        ----------
        bounding_boxes_on_images : list of augimg.augmentables.bbs.BoundingBoxesOnImages or augimg.augmentables.bbs.BoundingBoxesOnImages
        ...
        Returns
        -------
        list of augimg.augmentables.bbs.BoundingBoxesOnImage or augimg.augmentables.bbs.BoundingBoxesOnImage
            The augmented bounding boxes.
    ...
    def _augment_polygons_as_keypoints(self, polygons_on_images, random_state,
        ...
        .. warning::
            ...
            This method calls
            :func:`~augimg.augmenters.meta.Augmenter._augment_keypoints` and
        ...
        Parameters
        ----------
        polygons_on_images : list of augimg.augmentables.polys.PolygonsOnImage or augimg.augmentables.polys.PolygonsOnImage
            Polygons to augment. They may be changed in-place.
        ...
        recoverer : None or augimg.augmentables.polys._ConcavePolygonRecoverer
        ...
        Returns
        -------
        list of augimg.augmentables.polys.PolygonsOnImage or augimg.augmentables.polys.PolygonsOnImage
            The augmented polygons.
    ...
    def _augment_line_strings_as_keypoints(self, line_strings_on_images,
        ...
        Parameters
        ----------
        line_strings_on_images : list of augimg.augmentables.lines.LineStringsOnImages or augimg.augmentables.lines.LineStringsOnImages
            Line strings to augment. They may be changed in-place.
        ...
        Returns
        -------
        list of augimg.augmentables.lines.LineStringsOnImages or augimg.augmentables.lines.LineStringsOnImages
            The augmented line strings.
    ...
    def _augment_cbaois_as_keypoints(
        ...
        Parameters
        ----------
        cbaois : list of augimg.augmentables.bbs.BoundingBoxesOnImage or list of augimg.augmentables.polys.PolygonsOnImage or list of augimg.augmentables.lines.LineStringsOnImage or augimg.augmentables.bbs.BoundingBoxesOnImage or augimg.augmentables.polys.PolygonsOnImage or augimg.augmentables.lines.LineStringsOnImage
        ...
        parents : list of augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.augment_batch`.
        ...
        Returns
        -------
        list of augimg.augmentables.bbs.BoundingBoxesOnImage or list of augimg.augmentables.polys.PolygonsOnImage or list of augimg.augmentables.lines.LineStringsOnImage or augimg.augmentables.bbs.BoundingBoxesOnImage or augimg.augmentables.polys.PolygonsOnImage or augimg.augmentables.lines.LineStringsOnImage
    ...
    def _apply_to_polygons_as_keypoints(cls, polygons_on_images, func,
        ...
        func : callable
            ...
            :class:`~augimg.augmentables.kps.KeypointsOnImage` instances as its
            only parameter.
        ...
        random_state : None or augimg.random.RNG
    ...
    def augment(self, return_batch=False, hooks=None, **kwargs):
        ...
        :class:`~augimg.augmentables.batches.UnnormalizedBatch` and
        :func:`~augimg.augmenters.meta.Augmenter.augment_batch`. Hence, it
        supports the same datatypes as
        :class:`~augimg.augmentables.batches.UnnormalizedBatch`.
        ...
        * In **python <3.6** (if ``return_batch=False``):
        ...
        :class:`~augimg.augmentables.batches.UnnormalizedBatch` will be
        ...
        heatmaps : None or (N,H,W,C) ndarray or augimg.augmentables.heatmaps.HeatmapsOnImage or iterable of (H,W,C) ndarray or iterable of augimg.augmentables.heatmaps.HeatmapsOnImage, optional
            The heatmaps to augment.
            If anything else than
            :class:`~augimg.augmentables.heatmaps.HeatmapsOnImage`, then the
        ...
        segmentation_maps : None or (N,H,W) ndarray or augimg.augmentables.segmaps.SegmentationMapsOnImage or iterable of (H,W) ndarray or iterable of augimg.augmentables.segmaps.SegmentationMapsOnImage, optional
            The segmentation maps to augment.
            If anything else than
            :class:`~augimg.augmentables.segmaps.SegmentationMapsOnImage`, then
            the number of segmaps must match the number of images provided via
            parameter `images`. The number is contained either in ``N`` or the
            first iterable's size.

        keypoints : None or list of (N,K,2) ndarray or tuple of number or augimg.augmentables.kps.Keypoint or iterable of (K,2) ndarray or iterable of tuple of number or iterable of augimg.augmentables.kps.Keypoint or iterable of augimg.augmentables.kps.KeypointOnImage or iterable of iterable of tuple of number or iterable of iterable of augimg.augmentables.kps.Keypoint, optional
            ...
            :class:`~augimg.augmentables.kps.Keypoint` instances are used
            instead of tuples.
            If an ndarray, then ``N`` denotes the number of images and ``K``
            the number of keypoints on each image.
            If anything else than
            :class:`~augimg.augmentables.kps.KeypointsOnImage` is provided, then
        ...
        bounding_boxes : None or (N,B,4) ndarray or tuple of number or augimg.augmentables.bbs.BoundingBox or augimg.augmentables.bbs.BoundingBoxesOnImage or iterable of (B,4) ndarray or iterable of tuple of number or iterable of augimg.augmentables.bbs.BoundingBox or iterable of augimg.augmentables.bbs.BoundingBoxesOnImage or iterable of iterable of tuple of number or iterable of iterable augimg.augmentables.bbs.BoundingBox, optional
            The bounding boxes to augment.
        ...
        polygons : None or (N,#polys,#points,2) ndarray or augimg.augmentables.polys.Polygon or augimg.augmentables.polys.PolygonsOnImage or iterable of (#polys,#points,2) ndarray or iterable of tuple of number or iterable of augimg.augmentables.kps.Keypoint or iterable of augimg.augmentables.polys.Polygon or iterable of augimg.augmentables.polys.PolygonsOnImage or iterable of iterable of (#points,2) ndarray or iterable of iterable of tuple of number or iterable of iterable of augimg.augmentables.kps.Keypoint or iterable of iterable of augimg.augmentables.polys.Polygon or iterable of iterable of iterable of tuple of number or iterable of iterable of iterable of tuple of augimg.augmentables.kps.Keypoint, optional
            The polygons to augment.
            ...
            a single image:
              ...
              * ``augimg.augmentables.polys.Polygon``
              * ``iterable of tuple of number``
              * ``iterable of augimg.augmentables.kps.Keypoint``

            The following datatypes will be interpreted as multiple polygons
            on a single image:

              * ``augimg.augmentables.polys.PolygonsOnImage``
              * ``iterable of augimg.augmentables.polys.Polygon``
              * ``iterable of iterable of tuple of number``
              * ``iterable of iterable of augimg.augmentables.kps.Keypoint``
              * ``iterable of iterable of augimg.augmentables.polys.Polygon``
            ...
            multiple images:
              ...
              * ``iterable of iterable of iterable of tuple of augimg.augmentables.kps.Keypoint``

        line_strings : None or (N,#lines,#points,2) ndarray or augimg.augmentables.lines.LineString or augimg.augmentables.lines.LineStringOnImage or iterable of (#polys,#points,2) ndarray or iterable of tuple of number or iterable of augimg.augmentables.kps.Keypoint or iterable of augimg.augmentables.lines.LineString or iterable of augimg.augmentables.lines.LineStringOnImage or iterable of iterable of (#points,2) ndarray or iterable of iterable of tuple of number or iterable of iterable of augimg.augmentables.kps.Keypoint or iterable of iterable of augimg.augmentables.lines.LineString or iterable of iterable of iterable of tuple of number or iterable of iterable of iterable of tuple of augimg.augmentables.kps.Keypoint, optional
            The line strings to augment.
            See `polygons`, which behaves similarly.

        return_batch : bool, optional
            Whether to return an instance of
            :class:`~augimg.augmentables.batches.UnnormalizedBatch`. If the
        ...
        Returns
        -------
        tuple or augimg.augmentables.batches.UnnormalizedBatch
        ...
        :class:`~augimg.augmentables.batches.UnnormalizedBatch` from which the
    ...
    def __call__(self, *args, **kwargs):
        """Alias for :func:`~augimg.augmenters.meta.Augmenter.augment`."""
    ...
    def pool(self, processes=None, maxtasksperchild=None, seed=None):
        ...
        processes : None or int, optional
            Same as in :func:`~augimg.multicore.Pool.__init__`.
        ...
        maxtasksperchild : None or int, optional
            Same as for :func:`~augimg.multicore.Pool.__init__`.
        ...
        >>> import augimg as ia
        >>> import augimg.augmenters as iaa
        >>> from augimg.augmentables.batches import Batch
        >>>
        >>> aug = iaa.Add(1)
    ...
    def show_grid(self, images, rows, cols):
        ...
        This calls :func:`~augimg.augmenters.meta.Augmenter.draw_grid` and
    ...
    def to_deterministic(self, n=None):
        ...
        n : None or int, optional
            ...
            If ``None`` then only one :class:`~augimg.augmenters.meta.Augmenter`
            instance will be returned.
            If ``1`` or higher, a list containing ``n``
            :class:`~augimg.augmenters.meta.Augmenter` instances will be
            returned.

        Returns
        -------
        augimg.augmenters.meta.Augmenter or list of augimg.augmenters.meta.Augmenter
    ...
    def _to_deterministic(self):
        ...
        :func:`~augimg.augmenters.meta.to_deterministic`. This function is
        expected to return a single new deterministic
        :class:`~augimg.augmenters.meta.Augmenter` instance of this augmenter.

        Returns
        -------
        det : augimg.augmenters.meta.Augmenter
    ...
    @ia.deprecated("augimg.augmenters.meta.Augmenter.seed_")
    def reseed(self, random_state=None, deterministic_too=False):
        """Old name of :func:`~augimg.augmenters.meta.Augmenter.seed_`.
    ...
    def seed_(self, entropy=None, deterministic_too=False):
        ...
        :func:`~augimg.augmenters.meta.Augmenter.localize_random_state`
        does.
        ...
        :class:`~augimg.augmenters.meta.Augmenter` instance to a
        ...
        Parameters
        ----------
        entropy : None or int or augimg.random.RNG or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.SeedSequence or numpy.random.RandomState, optional
    ...
    def localize_random_state(self, recursive=True):
        ...
        recursive : bool, optional
            See
            :func:`~augimg.augmenters.meta.Augmenter.localize_random_state_`.

        Returns
        -------
        augimg.augmenters.meta.Augmenter
    ...
    def localize_random_state_(self, recursive=True):
        ...
        :func:`~augimg.augmenters.meta.Augmenter.to_deterministic`.
        Only when you copy RNGs (via
        :func:`~augimg.augmenters.meta.Augmenter.copy_random_state`),
    ...
    def copy_random_state(self, source, recursive=True, matching="position",
        ...
        Parameters
        ----------
        source : augimg.augmenters.meta.Augmenter
            See :func:`~augimg.augmenters.meta.Augmenter.copy_random_state_`.
    ...
    def copy_random_state_(self, source, recursive=True, matching="position",
        ...
        source : augimg.augmenters.meta.Augmenter
            ...
            :class:`~augimg.augmenters.meta.Sequential`).
    ...
    def get_parameters(self):
        ...
        list
            ...
            of :class:`~augimg.parameters.StochasticParameter`, but not
            guaranteed to be).
    ...
    def get_children_lists(self):
        ...
        ``[[A1, A2], [B1, B2, B3]]``
        for a call to
        :func:`~augimg.augmenters.meta.Augmenter.get_children_lists` and
        ``A2`` is removed inplace from ``[A1, A2]``, then the children lists
        of ``IfElse(...)`` must also change to ``[A1], [B1, B2, B3]``. This
        is used in
        :func:`~augimg.augmeneters.meta.Augmenter.remove_augmenters_`.

        Returns
        -------
        list of list of augimg.augmenters.meta.Augmenter
    ...
    # TODO this can be simplified using augimg.augimg.flatten()?
    def get_all_children(self, flat=False):
        ...
        Returns
        -------
        list of augimg.augmenters.meta.Augmenter
    ...
    def find_augmenters(self, func, parents=None, flat=True):
        ...
        func : callable
            ...
            :class:`~augimg.augmenters.meta.Augmenter` instance and a list of
            parent :class:`~augimg.augmenters.meta.Augmenter` instances and
    ...
    def find_augmenters_by_name(self, name, regex=False, flat=True):
        ...
        flat : bool, optional
            See :func:`~augimg.augmenters.meta.Augmenter.find_augmenters`.

        Returns
        -------
        augmenters : list of augimg.augmenters.meta.Augmenter
            Nested list if `flat` was set to ``False``.
            Flat list if `flat` was set to ``True``.
    ...
    def remove_augmenters(self, func, copy=True, identity_if_topmost=True,
        ...
        identity_if_topmost : bool, optional
            ...
            instance of :class:`~augimg.augmenters.meta.Noop` (i.e. an
        ...
        Returns
        -------
        augimg.augmenters.meta.Augmenter or None
    ...
    def remove_augmenters_inplace(self, func, parents=None):
        """Old name for :func:`~augimg.meta.Augmenter.remove_augmenters_`.
    ...
    def remove_augmenters_(self, func, parents=None):
        ...
        :func:`~augimg.augmenters.meta.remove_augmenters` with
        ...
        ----------
        func : callable
            See :func:`~augimg.augmenters.meta.Augmenter.remove_augmenters`.

        parents : None or list of augimg.augmenters.meta.Augmenter, optional
            List of parent :class:`~augimg.augmenters.meta.Augmenter` instances
...
class Sequential(Augmenter, list):
    ...
    .. note::
        ...
        You are *not* forced to use :class:`~augimg.augmenters.meta.Sequential`
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    children : augimg.augmenters.meta.Augmenter or list of augimg.augmenters.meta.Augmenter or None, optional
    ...
    seed : None or int or augimg.random.RNG or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.SeedSequence or numpy.random.RandomState, optional
        See :func:`~augimg.augmenters.meta.Augmenter.__init__`.
    ...
    Create a :class:`~augimg.augmenters.meta.Sequential` that always first
    ...
    Create a :class:`~augimg.augmenters.meta.Sequential` that sometimes first
    ...
    def get_parameters(self):
        """See :func:`~augimg.augmenters.meta.Augmenter.get_parameters`."""
    ...
    def get_children_lists(self):
        """See :func:`~augimg.augmenters.meta.Augmenter.get_children_lists`."""
        return [self]
...
class SomeOf(Augmenter, list):
    ...
    This augmenter is similar to :class:`~augimg.augmenters.meta.Sequential`,
    ...
    *per batch* (similar to :class:`~augimg.augmenters.meta.Sequential`).
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    n : int or tuple of int or list of int or augimg.parameters.StochasticParameter or None, optional
    ...
    def add(self, augmenter):
        ...
        Parameters
        ----------
        augmenter : augimg.augmenters.meta.Augmenter
            The augmenter to add.
...
class OneOf(SomeOf):
    ...
    **Supported dtypes**:
    ...
    See :class:`augimg.augmenters.meta.SomeOf`.

    Parameters
    ----------
    children : augimg.augmenters.meta.Augmenter or list of augimg.augmenters.meta.Augmenter
...
class Sometimes(Augmenter):
    ...
    :class:`~augimg.augmenters.meta.Sometimes`.
    ...
    **Supported dtypes**:
    ...
    Parameters
    ----------
    p : float or augimg.parameters.StochasticParameter, optional
    ...
    then_list : None or augimg.augmenters.meta.Augmenter or list of augimg.augmenters.meta.Augmenter, optional
        Augmenter(s) to apply to `p%` percent of all images.
        If this is a list of augmenters, it will be converted to a
        :class:`~augimg.augmenters.meta.Sequential`.

    else_list : None or augimg.augmenters.meta.Augmenter or list of augimg.augmenters.meta.Augmenter, optional
...
class Noop(Identity):
    ...
    **Supported dtypes**:
    ...
    See :class:`~augimg.augmenters.meta.Identity`.
...
class Lambda(Augmenter):
    ...
    **Supported dtypes**:
    ...
    func_images : None or callable, optional
        ...
        :func:`~augimg.augmenters.meta.Augmenter._augment_images`.
    ...
    func_heatmaps : None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_heatmaps`.
    ...
    func_segmentation_maps : None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_segmentation_maps`.
    ...
    func_keypoints : None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_keypoints`.
    ...
    func_bounding_boxes : "keypoints" or None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_bounding_boxes`.
    ...
    func_polygons : "keypoints" or None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_polygons`.
    ...
    func_line_strings : "keypoints" or None or callable, optional
        ...
        This is essentially the interface of
        :func:`~augimg.augmenters.meta.Augmenter._augment_line_strings`.
    ...
    def _augment_heatmaps(self, heatmaps, random_state, parents, hooks):
        if self.func_heatmaps is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "augimg.HeatmapsOnImage instances, got %s." % (
                    type(result),))
            only_heatmaps = all([
    ...
    def _augment_segmentation_maps(self, segmaps, random_state, parents, hooks):
        if self.func_segmentation_maps is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "list of augimg.SegmentationMapsOnImage() instances, "
                "got %s." % (type(result),))
            only_segmaps = all([
    ...
    def _augment_keypoints(self, keypoints_on_images, random_state, parents,
        ...
        if self.func_keypoints is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "augimg.augmentables.kps.KeypointsOnImage instances, "
                "got %s." % (type(result),))
            only_keypoints = all([
    ...
    def _augment_polygons(self, polygons_on_images, random_state, parents,
        ...
        if self.func_polygons is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "augimg.augmentables.polys.PolygonsOnImage instances, "
                "got %s." % (type(result),))
            only_polygons = all([
    ...
    def _augment_line_strings(self, line_strings_on_images, random_state,
        ...
        if self.func_line_strings is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "augimg.augmentables.lines.LineStringsOnImage instances, "
                "got %s." % (type(result),))
            only_ls = all([
                isinstance(el, ia.LineStringsOnImage) for el in result])
            assert only_ls, (
                "Expected callback function for line strings to return list of "
                "augimg.augmentables.lines.LineStringsOnImages instances, "
                "got %s." % ([type(el) for el in result],))
            return result
    ...
    def _augment_bounding_boxes(self, bounding_boxes_on_images, random_state,
        ...
        if self.func_bounding_boxes is not None:
            ...
            assert ia.is_iterable(result), (
                ...
                "of augimg.augmentables.bbs.BoundingBoxesOnImage instances, "
                "got %s." % (type(result),))
            only_bbs = all([
...
class AssertShape(Lambda):
    ...
    **Supported dtypes**:
    ...
    shape : tuple
        ...
        :func:`~augimg.augmenters.meta.Augmenter.augment_image`, then ``N`` is
    ...
    check_heatmaps : bool, optional
        ...
        :class:`~augimg.augmentables.heatmaps.HeatmapsOnImage` instance
    ...
    check_segmentation_maps : bool, optional
        ...
        :class:`~augimg.augmentables.segmaps.SegmentationMapOnImage` instance
        its array's height and width will be verified as ``H`` and ``W``,
        but not the channel count.
    ...
    check_keypoints : bool, optional
        ...
        :class:`~augimg.augmentables.kps.KeypointsOnImage` instance the
    ...
    check_bounding_boxes : bool, optional
        ...
        :class:`~augimg.augmentables.bbs.BoundingBoxesOnImage` instance the
        ``.shape`` attribute, i.e. the shape of the corresponding image.
    ...
    check_polygons : bool, optional
        ...
        :class:`~augimg.augmentables.polys.PolygonsOnImage` instance the
        ``.shape`` attribute, i.e. the shape of the corresponding image.
    ...
    check_line_strings : bool, optional
        ...
        :class:`~augimg.augmentables.lines.LineStringsOnImage` instance the
        ``.shape`` attribute, i.e. the shape of the corresponding image.
...
class ChannelShuffle(Augmenter):
    ...
    **Supported dtypes**:
    ...
    p : float or augimg.parameters.StochasticParameter, optional
        ...
        :class:`~augimg.parameters.StochasticParameter` that returns ``0`` s
        and ``1`` s.

    channels : None or augimg.ALL or list of int, optional
        Which channels are allowed to be shuffled with each other.
        If this is ``None`` or ``augimg.ALL``, then all channels may be
    ...
    def __init__(self, p=1.0, channels=None,
        ...
        assert valid_channels, (
            "Expected None or augimg.ALL or list of int, got %s." % (
                type(channels),))
        self.channels = channels
...
def shuffle_channels(image, random_state, channels=None):
    ...
    **Supported dtypes**:
    ...
    if is_all_channels:
        ...
        # augimg.ALL, so don't simply move the assignment outside of the
        # if/else
