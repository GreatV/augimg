"""Collection of basic functions used throughout augimg."""
...
def warn_deprecated(msg, stacklevel=2):
    ...
    The used warning is ``augimg.augimg.DeprecationWarning``.
...
def seed(entropy=None, seedval=None):
    """Set the seed of augimg's global RNG.

    The global RNG controls most of the "randomness" in augimg.
    ...
    .. note::
        ...
        future. The preferred way to seed `augimg` is via
        :func:`~augimg.random.seed`.
    ...
    import augimg.random
    augimg.random.seed(entropy)


@deprecated("augimg.random.normalize_generator")
def normalize_random_state(random_state):
    ...
    random_state : None or int or numpy.random.Generator or numpy.random.BitGenerator or numpy.random.bit_generator.SeedSequence or numpy.random.RandomState
        See :func:`~augimg.random.normalize_generator`.
    ...
    """
    import augimg.random
    return augimg.random.normalize_generator_(random_state)


@deprecated("augimg.random.get_global_rng")
def current_random_state():
    """Get or create the current global RNG of augimg.
    ...
    """
    import augimg.random
    return augimg.random.get_global_rng()


@deprecated("augimg.random.convert_seed_to_rng")
def new_random_state(seed=None, fully_random=False):
    ...
    if seed is None:
        if fully_random:
            return augimg.random.RNG.create_fully_random()
        return augimg.random.RNG.create_pseudo_random_()
    return augimg.random.RNG(seed)
...
def dummy_random_state():
    ...
    """
    import augimg.random
    return augimg.random.RNG(1)


@deprecated("augimg.random.copy_generator_unless_global_rng")
def copy_random_state(random_state, force_copy=False):
    ...
    import augimg.random
    if force_copy:
        return augimg.random.copy_generator(random_state)
    return augimg.random.copy_generator_unless_global_generator(random_state)


@deprecated("augimg.random.derive_generator_")
def derive_random_state(random_state):
    ...
    """
    import augimg.random
    return augimg.random.derive_generator_(random_state)


@deprecated("augimg.random.derive_generators_")
def derive_random_states(random_state, n=1):
    ...
    """
    import augimg.random
    return augimg.random.derive_generators_(random_state, n=n)


@deprecated("augimg.random.advance_generator_")
def forward_random_state(random_state):
    ...
    """
    import augimg.random
    augimg.random.advance_generator_(random_state)
...
def pool(arr, block_size, func, pad_mode="constant", pad_cval=0,
    ...
    **Supported dtypes**:
    ...
    pad_mode : str, optional
        ...
        without remainder. See :func:`~augimg.augimg.pad` for details.
...
def avg_pool(arr, block_size, pad_mode="reflect", pad_cval=128,
    ...
    **Supported dtypes**:
        ...
        See :func:`~augimg.augimg.pool`.

    Parameters
    ----------
    arr : (H,W) ndarray or (H,W,C) ndarray
        Image-like array to pool.
        See :func:`~augimg.augimg.pool` for details.
    ...
    pad_mode : str, optional
        Padding mode to use if the array cannot be divided by `block_size`
        without remainder.
        See :func:`~augimg.augimg.pad` for details.
...
def _avg_pool_uint8(arr, block_size, pad_mode="reflect", pad_cval=128):
    from augimg.augmenters.size import pad_to_multiples_of
...
def show_grid(images, rows=None, cols=None):
    ...
    This will show a window of the results of :func:`~augimg.augimg.draw_grid`.

    **Supported dtypes**:

        minimum of (
            :func:`~augimg.augimg.draw_grid`,
            :func:`~augimg.augimg.imshow`
        )

    Parameters
    ----------
    images : (N,H,W,3) ndarray or iterable of (H,W,3) array
        See :func:`~augimg.augimg.draw_grid`.
...
def imshow(image, backend=IMSHOW_BACKEND_DEFAULT):
    ...
    **Supported dtypes**:
    ...
    if backend == "cv2":
        ...
        win_name = "augimg-default-window"
        ...
        fig.canvas.set_window_title("augimg.imshow(%s)" % (image.shape,))
...
def apply_lut(image, table):
    ...
    **Supported dtypes**:
        ...
        See :func:`~augimg.augimg.apply_lut_`.
...
def _mark_moved_class_or_function(class_name_old, module_name_new,
    ...
    def _func(*args, **kwargs):
        import importlib
        warn_deprecated(
            "Using augimg.augimg.%s is deprecated. Use %s.%s instead." % (
...
MOVED = [
    ("Keypoint", "augimg.augmentables.kps", None),
    ("KeypointsOnImage", "augimg.augmentables.kps", None),
    ("BoundingBox", "augimg.augmentables.bbs", None),
    ("BoundingBoxesOnImage", "augimg.augmentables.bbs", None),
    ("Polygon", "augimg.augmentables.polys", None),
    ("PolygonsOnImage", "augimg.augmentables.polys", None),
    ("MultiPolygon", "augimg.augmentables.polys", None),
    ("_ConcavePolygonRecoverer", "augimg.augmentables.polys", None),
    ("HeatmapsOnImage", "augimg.augmentables.heatmaps", None),
    ("SegmentationMapsOnImage", "augimg.augmentables.segmaps", None),
    ("Batch", "augimg.augmentables.batches", None),
    ("BatchLoader", "augimg.multicore", None),
    ("BackgroundAugmenter", "augimg.multicore", None),
    ("compute_geometric_median", "augimg.augmentables.kps", None),
    ("_convert_points_to_shapely_line_string", "augimg.augmentables.polys",
     None),
    ("_interpolate_point_pair", "augimg.augmentables.polys", None),
    ("_interpolate_points", "augimg.augmentables.polys", None),
    ("_interpolate_points_by_max_distance", "augimg.augmentables.polys", None),
    ("pad", "augimg.augmenters.size", None),
    ("pad_to_aspect_ratio", "augimg.augmenters.size", None),
    ("pad_to_multiples_of", "augimg.augmenters.size", None),
    ("compute_paddings_for_aspect_ratio", "augimg.augmenters.size",
     "compute_paddings_to_reach_aspect_ratio"),
    ("compute_paddings_to_reach_multiples_of", "augimg.augmenters.size", None),
    ("compute_paddings_to_reach_exponents_of", "augimg.augmenters.size", None),
    ("quokka", "augimg.data", None),
    ("quokka_square", "augimg.data", None),
    ("quokka_heatmap", "augimg.data", None),
    ("quokka_segmentation_map", "augimg.data", None),
    ("quokka_keypoints", "augimg.data", None),
    ("quokka_bounding_boxes", "augimg.data", None),
    ("quokka_polygons", "augimg.data", None),
]
